...
logging.basicConfig(
	...
	handlers=[
		...
		logging.FileHandler("extracao_conhecimento.log", encoding="utf-8"),
	],
)
...
class ScraperTaim:
	...
	def _respeitar_delay(self) -> None:
		...
		if elapsed < self.delay:
			...
			logger.debug(f"Aguardando {wait:.1f}s (politeness delay)")
			time.sleep(wait)
	def buscar_pagina(self, url: str, timeout: int = 15) -> FonteTexto:
		...
		try:
			...
			logger.error(f"  ✗ ConnectionError em {url}: {e}")
			...
			logger.error(f"  ✗ HTTPError {response.status_code} em {url}")
		except requests.exceptions.TooManyRedirects as e:
			fonte.erro = f"Excesso de redirecionamentos: {e}"
			logger.error(f"  ✗ TooManyRedirects em {url}")
		except requests.exceptions.ContentDecodingError as e:
			fonte.erro = f"Erro de decodificação: {e}"
			logger.error(f"  ✗ ContentDecodingError em {url}")
		except requests.exceptions.RequestException as e:
			fonte.erro = f"Erro genérico de requisição: {e}"
			logger.error(f"  ✗ RequestException em {url}: {e}")
		return fonte
	def parse_wikipedia(self, fonte: FonteTexto) -> FonteTexto:
		...
		try:
			...
			if not content_div:
				...
				logger.warning(f"  ⚠ Conteúdo não encontrado em {fonte.url}")
				return fonte
	...
	def parse_artigo_cientifico(self, fonte: FonteTexto) -> FonteTexto:
		...
		try:
			...
			for seletor in seletores_corpo:
				...
				if corpo:
					logger.debug(f"  Corpo encontrado via seletor: {seletor}")
					break
			if not corpo:
				...
				if not corpo:
					...
					logger.warning(f"  ⚠ Corpo não encontrado em {fonte.url}")
					return fonte
			...
			try:
				...
				if r.status_code == 200:
					...
					if bindings:
						...
						fontes.append(fonte)
						logger.info(
							f"	✓ {len(bindings)} resultado(s), "
							f"{len(paragrafos)} parágrafos gerados"
						)
					else:
						logger.warning(f"	⚠ Sem resultados para {q['name']}")
				elif r.status_code == 429:
					logger.warning("	⚠ Rate limited (429), pulando query")
				else:
					logger.error(f"	✗ HTTP {r.status_code}")
			except Exception as e:
				logger.error(f"	✗ Erro Wikidata ({q['name']}): {e}")
		return fontes
	def coletar_ibge(self) -> list[FonteTexto]:
		fontes: list[FonteTexto] = []
		logger.info("\n  --- IBGE API (Localidades) ---")
		for item in IBGE_API_URLS:
			logger.info(f"  Consultando IBGE: {item['name']}")
			try:
				self._respeitar_delay()
				...
				if r.status_code == 200:
					...
					fontes.append(fonte)
					logger.info(
						f"	✓ Município: {nome_mun}, "
						f"{len(paragrafos)} parágrafos gerados"
					)
				else:
					logger.error(f"	✗ HTTP {r.status_code}")
			except Exception as e:
				logger.error(f"	✗ Erro IBGE ({item['name']}): {e}")
		return fontes
	def coletar_todas_fontes(self) -> list[FonteTexto]:
		...
		logger.info(f"  Detalhamento por tipo:")
		logger.info(f"	Wikipedia:	 {sum(1 for f in fontes if 'wikipedia.org' in f.url and f.paragrafos)}")
		logger.info(f"	Artigos:	   {sum(1 for f in fontes if 'periodicos' in f.url or 'scielo' in f.url)}")
		logger.info(f"	Notícias:	  {sum(1 for f in fontes if 'g1.globo' in f.url or 'oeco' in f.url)}")
		logger.info(f"	Wikidata:	  {len(fontes_wikidata)}")
		logger.info(f"	IBGE:		  {len(fontes_ibge)}")
		return fontes
...
class ProcessadorNLP:
	def __init__(self, modelo: str = "pt_core_news_sm"):
		...
		try:
			...
			logger.info(f"  ✓ Modelo spaCy carregado: {modelo}")
	...
	def _configurar_entity_ruler(self) -> None:
		...
		logger.info(f"	EntityRuler configurado com {len(patterns)} padrões")
...
class ConsolidadorResultados:
	...
	def deduplicar_triplas(
		...
		logger.info(f"  Deduplicação: {len(triplas)} → {len(dedup)} triplas únicas")
		return dedup
	...
	def exportar_csv_triplas(self, triplas: list[TriplaExtraida]) -> Path:
		...
		try:
			...
			logger.info(f"  ✓ CSV exportado: {caminho}")
		except IOError as e:
...
def executar_pipeline() -> None:
	...
	if not fontes_validas:
		logger.error("Nenhuma fonte válida coletada. Abortando.")
		return
